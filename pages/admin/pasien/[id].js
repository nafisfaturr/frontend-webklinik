...
export default function PasienEdit() {
  ...
  const fetchPatientDetails = async () => {
    if (!id) return;
    try {
      const response = await fetch(`/api/pasien/show/${id}`);
          ...
          <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
            <div className="rounded-t bg-white mb-0 px-6 py-6">
              <div className="text-center flex justify-between">
                <h6 className="text-blueGray-700 text-xl font-bold">Edit Patient</h6>
                <button
            ...
            <div className="flex-auto px-4 lg:px-10 py-10 pt-0">
              <form>
                ...
                <div className="flex flex-wrap">
                  ...
                  <div className="w-full lg:w-6/12 px-4">
                    <div className="relative w-full mb-3">
                      <label className="block uppercase text-blueGray-600 text-xs font-bold mb-2" htmlFor="nomor_telepon">
                        Nomor Telepon
                      </label>
                      <input
                        type="text"
                        id="nomor_telepon"
                        name="nomor_telepon"
                        value={patientData.nomor_telepon}
                        onChange={handleChange}
                        className="border-0 px-3 py-3 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
                      />
                    </div>
